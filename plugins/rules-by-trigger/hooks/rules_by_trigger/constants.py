...
ADMIN_COMMAND = os.path.join(PLUGIN_ROOT, "bin", "rules-by-trigger")
...
VERIFY_OUTPUT_CUT_MARKER = "[...earlier output cut by the rules-by-trigger size limit...]\n"
...
# shipped default), then `~/.claude/rules-by-trigger/`, then each project scope.
...
SESSION_NOTICE = (
    ...
    "markdown files under `.claude/rules-by-trigger/` (project) and "
    "`~/.claude/rules-by-trigger/` (global), and they reach you AUTOMATICALLY: the "
    ...
    "or --global — or the rules-by-trigger:manage skill, which drives it for you."
)
...
TRUNCATION_NOTICES = {
    ...
    BRAZILIAN_PORTUGUESE:
        "\n[...regra truncada pelo limite de tamanho do rules-by-trigger...]",
}
...
BLOCK_REASON_TEMPLATE = (
    "rules-by-trigger: this tool call is blocked by the rule {name!r} "
...
RULES_OPEN_TAG = "<rules-by-trigger>"
RULES_CLOSE_TAG = "</rules-by-trigger>"
RULE_SEPARATOR = "---"
...
def warn(message):
    print(f"rules-by-trigger: {message}", file=sys.stderr)
